...
def drop_media_cache(mid):
    for f in os.listdir(MEDIA_CACHE_DIR):
        if f.startswith(mid + "-"):
            try: os.remove(os.path.join(MEDIA_CACHE_DIR,f))
            except Exception: pass
...
def serve_file(path, ct):
    ...
    r=Response(data,mimetype=ct);r.headers['Cache-Control']='public, max-age=86400';return r
...
def media_proxy():
    mid=request.args.get('id');kind=(request.args.get('kind') or 'image').lower()
