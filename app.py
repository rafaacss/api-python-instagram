...
def ig_get(id_or_path, fields):
    r = requests.get(
        f"{GRAPH_API_URL.rstrip('/')}/{id_or_path.lstrip('/')}",
...
def proxy_image_legacy():
    # 1) Se vier id direto, atende por aqui mesmo (evita undefined virar erro)
    mid  = (request.args.get('id') or '').strip()
    kind = (request.args.get('kind') or 'image').lower()
    if mid:
        return handle_media_proxy(mid, kind)

    # 2) Fluxo legado: url=/api/instagram/media_proxy?id=...
    ...
    # 3) Se for seu próprio media_proxy, extrai o id e serve
    ...
    # 4) Proxy simples para CDN do Instagram
    try:
...
def get_user_posts():
    ...
    def img_url(mid: str) -> str:
        # base para grid e lightbox
        return f"/api/instagram/media_proxy?id={mid}&kind=image"

    def make_cover(mid: str):
        return {
            "thumbnail": {"url": img_url(mid), "width": 1080.0, "height": 1920.0},
    ...
    for post in data.get("data", []):
        ...
        if ptype in ("IMAGE", "VIDEO"):
            ...
            media_items.append({
                ...
                "id": mid,
                "url": img_url(mid),
                # 👇 **IMPORTANTE**: o widget usa isso no lightbox.
                "images": [
                    {"url": img_url(mid)}  # pode adicionar mais variações se quiser
                ],
